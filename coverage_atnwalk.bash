...
declare -A GRAMMARS=(
  [mruby]=${HOME}/atnwalk/build/ruby/bin/server
  [sqlite3]=${HOME}/atnwalk/build/sqlite/bin/server
  [php]=${HOME}/atnwalk/build/php/bin/server
  [lua]=${HOME}/atnwalk/build/lua/bin/server
  [jerry]=${HOME}/atnwalk/build/javascript/bin/server
)
...
do
  ...
  do
    ...
    do
      ...
      nohup ${GRAMMARS[${target_name}]} ${RUN_TIMEOUT} > server.log 2>&1 &
      sleep 1
      j=1
      total_files=$(find ~/campaign/"${campaign_date}"/"${FUZZER}"/"${target_name}"/"${i}"/out/default/queue/ -maxdepth 1 -type f | wc -l)
      for f in $(find ~/campaign/"${campaign_date}"/"${FUZZER}"/"${target_name}"/"${i}"/out/default/queue/ -maxdepth 1 -type f)
      do
        printf '\rdecoding: %d / %d' ${j} ${total_files}
        cat "${f}" | $(dirname ${GRAMMARS[${target_name}]})/client -t ${RUN_TIMEOUT} -d > "${this_run_dir}/corpus/"$(basename "${f}")
        touch -r "${f}" "${this_run_dir}/corpus/"/$(basename "${f}")
        (( j++ ))
      done
      printf '\n'
      ...
      grep 'execs_done' ~/campaign/"${campaign_date}/"${FUZZER}"/"${target_name}"/"${i}/out/default/fuzzer_stats | cut -d':' -f2 | xargs > "${this_run_dir}"/"${FUZZER}"."${target_name}".${NEW_RUN_COUNT}.total_execs.txt
      find ~/campaign/"${campaign_date}/"${FUZZER}"/"${target_name}"/"${i}/out/default/crashes/ -maxdepth 1 -type f -printf x | wc -c > "${this_run_dir}"/"${FUZZER}"."${target_name}".${NEW_RUN_COUNT}.total_crashes.txt
